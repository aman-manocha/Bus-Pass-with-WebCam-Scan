...
class DatabaseUtility: 
        ...
        #def GetName(self):
         #       return self.RunCommand("SELECT Name FROM information where email id = 
        ...
        def AddEntryToTable(self,Na, Eid, Pas, Ph, Ad, Ah) :
		
                cmd = " INSERT INTO " + self.tableName + " (`Name`, `email id`, `Password`, `Phone No.`, `Address`, `Adhaar`)"
                cmd += " VALUES ('%s', '%s' , '%s' , '%s' , '%s' , '%s');" % (Na, Eid, Pas, Ph, Ad, Ah)
                self.RunCommand(cmd)
                self.cnx.commit()
